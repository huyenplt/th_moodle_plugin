...
class lib {
    ...
    public static function get_allcourseid_form($mform) {
		...
		$options = array(
			'multiple' => true,
			// 'noselectionstring' => get_string('choose_a_course', 'block_course_unenrollment'),
		);
		$mform->addElement('autocomplete', 'courseid', get_string('search', 'block_course_unenrollment'), $choice, $options);
		return $courses;
	}
}